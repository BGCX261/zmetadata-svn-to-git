...
import interfaces
from Products.Communities.interfaces.standardssetup import IStandardsSetup
...
class CommonSetup(ATFolder):
    ...
    def getMyFields(self):
        ...
        """
        names = []
        for field in self['CommonFields'].schema.fields():
            names += [field.getName()]
        return names
    ...
    def _renameAfterCreation(self,check_auto_id=True):
        """
        """
        self._setObject("commonFields", CommonFields("commonFields"))
        self["commonFields"].setTitle("Common Standard Setup")
