<xsl:stylesheet xmlns:xsl = "http://www.w3.org/1999/XSL/Transform" xmlns:gco="http://www.isotc211.org/2005/gco"
    ...
    <xsl:template match = "/">
	  ...
	  <xsl:apply-templates select="//language/CharacterString" mode="language"/>  	  	  
	  <xsl:apply-templates select="//gmd:contact/gmd:CI_ResponsibleParty/gmd:individualName/gco:CharacterString/text()" mode="owner"/>  	  	  	  
    </xsl:template>
