...
if workflow_action in transition_ids:
    try: 
        ...
        wfcontext = None
        statusMessage = "Error changing status: %s" %e
...
if statusMessage is None:
    statusMessage = _(u'Item state changed.')
    context.plone_utils.addPortalMessage(statusMessage)
else:
    context.plone_utils.addPortalMessage(statusMessage, 'error')
